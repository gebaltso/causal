...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan  8 16:30:50 2021
        ...
        if len(neig_comm)>1:   #说明此加入结点不在社区内部
           ...
           change_comm.add(lab)
            ...
            if len(neig_comm)==1:#说明结点在社区内部，或只与一个社区连接
               ...
               communitys.setdefault(v,max(communitys.values())+1)#新加结点未和其它结点有连接，分配新的社区标签
...
def edge_addition(addedges,communitys):#If the joining edge is within the community and will not cause changes in the community, it will not be processed, otherwise it will be marked
    change_comm=set()#The storage structure may find community tags that have changed
#    print addedges
#    print communitys    
    for item in addedges:        
        neig_comm=set()#Neighborhood label 
        neig_comm.add(communitys[item[0]])#Determine the communities where the nodes at both ends of one side are located
        neig_comm.add(communitys[item[1]])
        if len(neig_comm)>1:   #Explain that this joining edge is not in the community
           change_comm=change_comm | neig_comm
    return change_comm # Return to communities that may change,

def edge_deletion(deledges,communitys):#If deleting an edge may cause community changes within the community, it will not change outside the community
    change_comm=set()#The storage structure may find community tags that have changed    
...
# 社团划分**************************************************************
    ...
    print('End of community division')
    print(list_I)
...
G=G.to_undirected()
#初始图
print('T0 time slice network G0*********************************************')
nx.draw_networkx(G)
...
comm=CDID(G,0)   #初始社区 
#画社区
print('Community C0 in T0 time slice*********************************************')
...
#         G1.add_edge(int(temp[0]),int(temp[1]))
    ...
    print('T'+str(i-1)+'Time sliced ​​network G'+str(i-1)+'*********************************************')
    nx.draw_networkx(G2)
...
#    previous_nodes.add(1001)
    ...
    print ('Increase the node set as：',nodes_added)
...
#    print ('remove node',nodes_removed)
    ...
    print ('Increase the edge set to：',edges_added)
    edges_removed = set(G1.edges())-set(G2.edges())
    print ('Delete edge set is：',edges_removed)
...
#    print('subG',Gtemp.edges())
    if Gtemp.number_of_edges()<1:#社区未发生变化
        ...
        print('T'+str(i-1)+'Time slice subgraph delta_g'+str(i-1)+'*********************************************')
...
#        mergecomm=dict(unchcommunity, **getnewcomm )#格式为｛节点：社区｝
        ...
        print('T'+str(i-1)+'Time sliced ​​network community structure C'+str(i-1)+'*********************************************')
